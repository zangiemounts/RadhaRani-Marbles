...
const MesurementData = () => {
    ...
    const filteredData = user.filter(
        (item) => item.userData.firstName.toLowerCase().includes(value.toLowerCase()) || String(item.meaasurementid?.orderNo).toLowerCase().includes(value.toLowerCase()) 
    )
    ...
    const renderClient1 = row => {
        ...
        if (!row.cloudInterfactDisplaySectionId.length) {
            ...
            return <Avatar color={color} className='me-50' content={row.userData.firstName ? `${row.userData.firstName}` : 'Radharani'} initials />
        } else {
            return <Avatar color={color} className='me-50' content={row.userData.firstName ? `${row.userData.firstName} ${row.userData.lastName}` : 'Radharani'} initials />
        }
    }
    ...
    const apidata = localStorage.getItem('userIdA') ? "ViewEstimate" : "ViewEstimateEmployee"
    ...
    const redirect = async (id, id2, id3) => {
        ...
        sessionStorage.setItem("MesurementView", id)
        window.location.pathname = "/mesurementView"        
    }

    const redirect1 = async (id) => {
        setIsloading(true)
        sessionStorage.setItem("Mesurement", id)
    ...
    const data = [
        {
            ...
            cell: row => <Link to="#" onClick={() => redirect(row.meaasurementid?.cloudInterfactDisplaySectionId)}>{`#M${row.meaasurementid?.orderNo !== null ? row.meaasurementid?.orderNo : 0}`}</Link>
        },
        {
            name: 'Estimate No',
            sortable: false,
        ...
        },
        {
            ...
            minWidth: '120px',
            cell: row => {
                const dateStr = row?.bill?.createdOn
                const formattedDate = dateStr ? new Date(dateStr).toISOString().slice(0, 10) : 'N/A'
                ...
                return (
                    <div className='d-flex justify-content-left align-items-center'>
                        ...
                        <div className='d-flex flex-column'>
                            <h6 className='user-name text-truncate mb-0 text-wrap'>{name}</h6>
        ...
        },
        {
            ...
            minWidth: '150px',
            sortField: 'total',
            cell: row => <span>₹ {(row.bill?.payableAmount !== null ? row.bill.payableAmount.toFixed(2) : 0)}</span>
        },
        {
            name: 'Action',
            minWidth: '210px',
            cell: row => (
                <div className='column-action d-flex align-items-center'>
                    <Button onClick={() => redirect(row.meaasurementid?.cloudInterfactDisplaySectionId)} color="success" size="sm">  <Eye size={15} />View Measurement</Button>
                </div>
            )
        },
        {
            ...
            cell: row => (
                <div className='column-action d-flex align-items-center'>
                    {!Isloading ? < Button onClick={() => redirect1(row.meaasurementid?.cloudInterfactDisplaySectionId)} color="warning" size="sm"> <Edit3 size={23} /> Edit Mesurement</Button> : <Button color='success' disabled>
        ...
        },
        {
            ...
            cell: row => (
                <div className='column-action d-flex align-items-center'>
                    <Button onClick={() => rediresct2(row.apikey2)} color="info"><Eye size={15} /> View Estimate</Button>
        ...
        <div className='invoice-list-wrapper'>
            <Card>
                <div className='invoice-list-dataTable react-dataTable'>
                    {paginatedData.length < 0 ? (<div className="d-flex item-center p-5"> <Spinner color="success" /> <p className="m-1">Please wait wile data loading</p> </div>) : (
                        <DataTable
                            noHeader
                            pagination
                            sortServer
